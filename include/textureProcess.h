...
void drawTilemap(Texture2D sheet, Texture2D deco, Texture2D deco2) {
...
void drawPlayerFOV(Vector2 playerPos, float playerAngle) {
...
Vector2 castSingleRay(Vector2 start, float angleRad, float maxDist) {
...
void drawLightSystem(float playerAngle, RenderTexture2D lightMask, Camera2D camera) {
    ...
    Vector2 screenPlayer = Vector2Add(playerObject->pos, offset);
    // draw mask
    BeginTextureMode(lightMask);
        ...
        rlSetBlendFactors(0, 0x0303, 0x8006); 
            ...
            for (int i = 0; i < numRays; i++) {
                ...
                Vector2 hit1 = castSingleRay(playerObject->pos, a1, maxDist);
                Vector2 hit2 = castSingleRay(playerObject->pos, a2, maxDist);

                // shift coordinates so it looks correct
                screenPlayer = Vector2Add(playerObject->pos, offset);
